...
class DemoArUco : public jevois::StdModule
{
  public: 
    ...
    virtual void process(jevois::InputFrame && inframe) override
    {
      ...
      for (size_t i = 0; i < ids.size(); ++i)
      {
          ...
          // ---------------------------------------------------------
          // 2) Centre du marqueur (cx, cy)
          // ---------------------------------------------------------
          float cx = (corners[i][0].x + corners[i][2].x) * 0.5f;
          float cy = (corners[i][0].y + corners[i][2].y) * 0.5f;

          uint8_t* bx = reinterpret_cast<uint8_t*>(&cx);
          uint8_t* by = reinterpret_cast<uint8_t*>(&cy);

          payload.insert(payload.end(), bx, bx + 4);
          payload.insert(payload.end(), by, by + 4);

          // ---------------------------------------------------------
          // 3) tvecs = position 3D (tx, ty, tz)
          // ---------------------------------------------------------
          if (!tvecs.empty())
          {
              float tx = tvecs[i][0];
              ...
              uint8_t* btx = reinterpret_cast<uint8_t*>(&tx);
              uint8_t* bty = reinterpret_cast<uint8_t*>(&ty);
              uint8_t* btz = reinterpret_cast<uint8_t*>(&tz);

              payload.insert(payload.end(), btx, btx + 4);
              payload.insert(payload.end(), bty, bty + 4);
              payload.insert(payload.end(), btz, btz + 4);
          }
          else
          {
              // si pose OFF : mettre 0
              for (int k = 0; k < 12; k++) payload.push_back(0);
          }

          // ---------------------------------------------------------
          // 4) rvecs = rotation 3D (rx, ry, rz)
          // ---------------------------------------------------------
          if (!rvecs.empty())
          {
              float rx = rvecs[i][0];
              ...
              uint8_t* brx = reinterpret_cast<uint8_t*>(&rx);
              uint8_t* bry = reinterpret_cast<uint8_t*>(&ry);
              uint8_t* brz = reinterpret_cast<uint8_t*>(&rz);

              payload.insert(payload.end(), brx, brx + 4);
              payload.insert(payload.end(), bry, bry + 4);
              payload.insert(payload.end(), brz, brz + 4);
          }
          else
          {
              for (int k = 0; k < 12; k++) payload.push_back(0);
          ...
          // ---------------------------------------------------------
          // 5) Choix d’un code commande
          // ---------------------------------------------------------
          uint16_t cmd = 0x0201;  // "Pose ArUco detected"

          sendProtocolMessage(cmd, payload);
      }
    }
  ...
  protected:
    ...
    std::vector<uint8_t> buildProtocolMessage(uint16_t command,
        std::vector<uint8_t> const& payload)
    {
        ...
        // 2) Command (2 octets, little endian)
        msg.push_back(command & 0xFF);
        msg.push_back((command >> 8) & 0xFF);

        // 3) Payload length (2 octets)
        uint16_t plen = payload.size();
        msg.push_back(plen & 0xFF);
        msg.push_back((plen >> 8) & 0xFF);

        // 4) Payload
        msg.insert(msg.end(), payload.begin(), payload.end());

        // 5) Checksum XOR sur TOUT, y compris SOF
        uint8_t checksum = 0;
    ...
    // =============================================================
    // Fonction d’envoi série du message
    // =============================================================
    void sendProtocolMessage(uint16_t cmd, std::vector<uint8_t> const& payload)
    {
        std::vector<uint8_t> frame = buildProtocolMessage(cmd, payload);

        // Envoi binaire direct sur le port série JeVois
        this->sendSerial(frame.data(), frame.size());
    }
};
